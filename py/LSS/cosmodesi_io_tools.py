...
def read_clustering_positions_weights(distance, zlim =(0., np.inf), maglim=None, magkey='ABSMAG_R', weight_type='default', name='data', concatenate=False, option=None, region=None, cat_read=None, dat_cat=None, ran_cat=None,P0=None, **kwargs):
    #print(kwargs)
    #if 'GC' in region:
...
#                    cat_full = catalog_fn(ctype='full', name=name, **kwargs)
                ...
                if name=='data':
                    def _get_tab(cat_fn):
                        ...
                        return tab
                    positions_weights = [get_clustering_positions_weights(_get_tab(cat_fn), distance, zlim=zlim, maglim=maglim, magkey=magkey, weight_type=weight_type, name=name, option=option,P0=P0) for cat_fn in cat_fns]
                else:
                    positions_weights = [get_clustering_positions_weights(Table.read(cat_fn), distance, zlim=zlim, maglim=maglim, magkey=magkey, weight_type=weight_type, name=name, option=option,P0=P0) for cat_fn in cat_fns]
    ...
    if cat_read != None:
        def read_positions_weights(name):
            ...
            for reg in region:
                ...
                positions_weights = [get_clustering_positions_weights(cat_read, distance, zlim=zlim, maglim=maglim, magkey=magkey, weight_type=weight_type, name=name, option=option,P0=P0)]
                if name == 'data':
                    positions.append(positions_weights[0][0])
                    weights.append(positions_weights[0][1])
